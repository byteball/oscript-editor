export default `{
	/*
    Autonomous Agent like EOS Crowdsale on Ethereum
    ...
    * Each round is 1 hour long (for testing purposes) and a total of 1 million tokens can be claimed with each round. It is configurable by deploying new AA.
    * Amount of tokens each investor can claim depends on how much other investors invested during that same round. If nobody invested during some round, then no tokens will be issued from that round. Other users can pay the claiming fees of other investors as tokens will be sent to only investor address.
    * Crowdsale length is 720 hours (30 days), which means that the maximum amount of tokens ever issued will be 720 million. It is configurable by deploying new AA.
    * Founder can withdraw invested bytes anytime.
